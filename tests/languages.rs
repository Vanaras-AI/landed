...
fn an_arrow_function_bound_to_a_name_is_a_definition() {
    ...
    write(&dir, "src/main.test.ts", "import { unused } from \"./main\";\ntest(\"u\", () => { unused(); });\n");
    let s = scan::scan_crate(&dir).unwrap();
    let names: Vec<&str> = s.defs.iter().map(|d| d.name()).collect();
...
fn a_component_used_only_in_jsx_is_reachable() {
    ...
    write(&dir, "src/Chart.test.tsx", "import { Chart, Unused } from \"./Chart\";\ntest(\"c\", () => { Chart(); Unused(); });\n");
    let names = dead_names(&dir);
    assert!(!names.contains(&"Chart".to_string()), "JSX use missed: {names:?}");
...
fn typescript_publicness_is_export_not_a_naming_convention() {
    ...
    write(&dir, "src/i.test.ts", "test(\"i\", () => { publicThing(); internalThing(); });\n");
    let names = dead_names(&dir);
    assert!(!names.contains(&"publicThing".to_string()), "exports are roots: {names:?}");
    assert!(names.contains(&"internalThing".to_string()), "got {names:?}");
    std::fs::remove_dir_all(&dir).ok();
}
...
fn the_entry_module_a_host_loads_has_root_exports() {
    ...
    let names = dead_names(&dir);
    assert!(!names.contains(&"activate".to_string()), "host entry is a root: {names:?}");
    assert!(!names.contains(&"boot".to_string()), "reached through activate: {names:?}");
...
fn a_typescript_application_is_told_from_a_library() {
    for (name, manifest, index, want_app) in [
        ...
        ("ext", r#"{"name":"a","engines":{"vscode":"^1.0.0"}}"#, false, true),
